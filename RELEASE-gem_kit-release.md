# Releasing gem_kit-release

The whole process, in order:

```sh
bin/test                                           # 1. green suite
gem kit bump --gem gem_kit-release minor           # 2. bump  (prints what to run next)
gem kit changelog --gem gem_kit-release --write    # 3. write the entry
gem kit changelog --gem gem_kit-release <VERSION>  # 4. check it
git commit -am "Release ..."                       # 5. commit the bump + changelog
gem kit release --gem gem_kit-release              # 6. build and push
gem kit tag --gem gem_kit-release --push           # 7. tag it
```
...
gem_kit-release is [semver](https://semver.org/). The version lives in one place —
`lib/gem_kit/release/version.rb` — and is only ever changed by `gem kit bump --gem gem_kit-release`.
...
```sh
gem kit bump --gem gem_kit-release <major|minor|patch>
```

Rewrites `lib/gem_kit/release/version.rb` — by rendering its `.erb` template if there is
...
```sh
gem kit changelog --gem gem_kit-release --write
```
...
It reads the commits since the last tag and edits `CHANGELOG-gem_kit-release.md` only.
...
gem kit changelog --gem gem_kit-release 0.1.0       # format, plus "is this version ready?"
```
...
list. Every problem is reported as `CHANGELOG-gem_kit-release.md:<line> <what>`.
...
```sh
gem kit release --gem gem_kit-release           # or: gem kit release --gem gem_kit-release --dry-run, which is the CI check
```
...
Creates `gem_kit-release-v0.1.0`, refusing if it already exists. The tag is also what
...
**Published a broken gem.** Don't delete it — `gem yank gem_kit-release -v X.Y.Z` if
...
re-run `gem kit release --gem gem_kit-release`; there is no need to un-bump.

**Changelog written for the wrong version.** Edit the heading and re-run
`gem kit changelog --gem gem_kit-release <version>`. Nothing downstream caches it.

## See also

- [DEPRECATIONS.md](DEPRECATIONS.md) — the deprecation policy the gates enforce.
- [CHANGELOG-gem_kit-release.md](CHANGELOG-gem_kit-release.md).
